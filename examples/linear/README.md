...
```bash
pip install -e .                    # SDK
pip install -e examples/linear      # this plugin
```
...
```bash
opensre-plugin register examples/linear
```
...
| Schema validation fails | Run `opensre-plugin validate examples/linear` and fix reported paths |
| 401 from Linear | Check API key permissions |

## Offline alternative

For demos without a Linear account, use [`examples/mock`](../mock/) instead.
